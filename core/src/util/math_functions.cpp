...
// A(M,K) * B(K, N) = C(M, N)
template <typename Dtype>
void gemm_cpu(bool bTransA, bool bTransB, const int M, const int N, const int K, 
    ...
    for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {
      for (int kernel_col = 0; kernel_col < kernel_w; kernel_col++) {
        ...
        for (int output_rows = output_h; output_rows; output_rows--) {	
          // If (input_row >= height || input_row < 0), that means it is pad and should be setted to zero.
...
//////////////////////////////////////////////////
// 在deconv中，该函数用在Forward过程，output为以实际输出进行卷积后应得到的维度，
// 与data_col对应，即由该维度的矩阵可直接化为该col；入参channels/height/width为实际输出维度，与data_im对应
template <typename Dtype>
