...
namespace dlex_cnn
{
	...
	public:
		...
		virtual int setOpParam(const std::string &opParamStr) { return -1; };

		// Generate operator's parameter string for model saving.
		virtual std::string genOpParamStr() const { return ""; };

		virtual int inferOutShape(std::vector<int> &inShape, std::vector<int> &outShape) { return -1; };
		...
		virtual int allocOpBuf4Train(const std::vector<int> &inShape, const std::vector<int> &outShape) { return -1; };
