...
namespace dlex_cnn {
	...
	void ConvolutionOpTest<Dtype>::forward()
	{
		...
		ConvolutionParam.kernel_num = 2;
		ConvolutionParam.kernel_h = 2;
		ConvolutionParam.kernel_w = 2;
		ConvolutionParam.pad_h = 0;
		ConvolutionParam.pad_w = 0;
		...
		conv1->setOpParam(ConvolutionParam);
		...
		matrixShow_float("C", (float *)inDiff[0]->getData(), inShape[tind::eNum], inShape[tind::eChannels], inShape[tind::eHeight], inShape[tind::eWidth]);

		const std::vector<int> kernelShape = inDataVec[1]->getShape();
		matrixShow_float("weight gradient", (float *)conv1->gradient_[0]->getData(), kernelShape[tind::eNum], kernelShape[tind::eChannels], kernelShape[tind::eHeight], kernelShape[tind::eWidth]);

		const std::vector<int> biasShape = inDataVec[2]->getShape();
		matrixShow_float("bias gradient", (float *)conv1->gradient_[1]->getData(), biasShape[tind::eNum], biasShape[tind::eChannels], biasShape[tind::eHeight], biasShape[tind::eWidth]);
	}
...
void testConv()
{
	dlex_cnn::ConvolutionOpTest<float> convTest;
	convTest.forward();
}
#endif